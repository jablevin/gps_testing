...
# global lidar_loc
# global resampled_loc
# global sigma_g
# global sat_bool

# colors = {'lidar':'darkorange', 'imu':'crimson', 'gps':'lime', 'ins':'blue'}
# plt.figure()
# plt.plot([1],[1], label = 'Lidar', color = colors['lidar'])
# plt.plot([1],[1], label = 'Fitted Points', color = 'red')
# plt.plot([1],[1], label = 'INS', color = colors['ins'])
# plt.scatter([1],[1], label = 'Start', color = 'green')
# plt.scatter([1],[1], label = 'End', color = 'red')
# plt.scatter([1],[1], label = 'Actual Start/End', color = 'purple')
# plt.legend()
# plt.show()
# input('break')
...
for run_name in [#'20180529_3433Outside_RadDet_Run009']:
        ...
        # '20180627_50Cto70LoadingDock_antennaElevated_BarometricCorrection_run003',
        # '20180627_50Cto70LoadingDock_antennaOnBox_run001',
        '20180627_70LoadingDockto50C_antennaElevated_run002',
        # '20180627_70LoadingDockto50C_antennaOnBox_BarometricCorrection_run004'

]:

    start = time.time()
    # run_name = '20180626_run02_antenna_shielded_elevated'
    ...
    if run_name.startswith('20180626'):
        name = run_name[-31:]
    if run_name.startswith('20180627') and run_name.endswith('3'):
        name = run_name[-43:]
    if run_name.startswith('20180627') and run_name.endswith('1'):
        name = run_name[-19:]
    if run_name.startswith('20180627') and run_name.endswith('2'):
        name = run_name[-22:]
    if run_name.startswith('20180627') and run_name.endswith('4'):
        name = run_name[-40:]
    ...
    def fit(params, lidar_loc, resampled_loc, sat_bool, sigma_g):
        x, y, theta = params
    ...
    flag1 = time.time()
    new_t = np.linspace(0, ins_time[-1], int(ins_time[-1]))
    ...
    if plot:
        ...
        temp_yaw = res_yaw
        for i in range(len(res_yaw)):
            if np.sign(res_yaw[i]) == -1:
                temp_yaw[i] = 360 + res_yaw[i]
    ...
    if lidar_active:
        ...
        flag3 = time.time()
        results = optimization.minimize(fit, initial_guess, method = methods[8],
    ...
    if plot:
        ...
        for j, lab in enumerate([str(sections[0]), str(sections[1]), str(sections[2]), str(sections[3]), str(sections[4])]):
            cbar.ax.text(-.5, (2 * j ) / 8.0, lab[:3], ha='center', va='center', fontsize = 14)
